...
def init_word_table():
    ...
    word_table_init[-1] = np.zeros(embed_size)
...
def news_iterator_v1(data, batch_size, num_step, max_news_sequence, max_word_sequence, flag):
    with open(os.path.join(vec_path, "vocab_vec.json"), 'r') as f:
        dit = json.load(f)
        word2id = {word: i for i, word in enumerate(dit.keys())}

    preprocess_path = f'train_preprocess_{info}.pkl'
    if flag == 'valid':
        preprocess_path = f'valid_preprocess_{info}.pkl'
    if flag == 'test':
        preprocess_path = f'test_preprocess_{info}.pkl'
    preprocess_path = os.path.join(root_path, preprocess_path)
    x_all = []
    y_all = []
    y_step_all = []
    ...
    for i in tqdm(range(len(data[0]))):
        ...
        for ind in range(len(prices)):
            ...
            if len(str(news_lis[ind])) < 4:
                daily_news_rep = np.zeros(embed_size)
            else:
...
def series_feature_union():
    ...
    for file in files:
        ...
        history.append(df[(df.date >= training_begin_date) & (df.date < validing_end_date)][label_col].values*times)
        test.append(df[(df.date >= validing_end_date) & (df.date < testing_end_date)][label_col].values*times)
    return history, test
